...
import { renderDomList } from './utils';

console.log('### wokroutPlay > getWorkoutPlayData');

export function setWorkoutPlayHeader() {
  //운동실행화면[헤더]
  const workoutPlayHeaderDom = document.getElementById(
    'targetWorkoutPlayHeader',
  );

  //운동 루틴, 운동 data
  const { workoutPlayRoutine, workoutPlayList } = getWorkoutPlayData();
  const sortedWorkoutPlayList = workoutPlayList.sort(
  ...
  //init header
  const domList = new DOMParser().parseFromString(
    workoutPlayHeaderTitleTemplate,
    'text/html',
  ).body.childNodes;
  ...
  const currentWorkoutSecondDom = document.getElementById(
    'currentWorkoutSecond',
  );
...
export function setWorkoutPlayToolbar() {
  ...
  */
  ...
  const {
    ...
    leftSecond: workoutLeftSecond,
  } = getWorkoutTotalTime(workoutPlayRoutine.index);
  ...
  const formatTimeWorkoutLeftSecond = formatStringWorkoutLeftSecond.padStart(
    2,
    '0',
  );
  ...
  const workoutPlayToolbarDomList = new DOMParser().parseFromString(
    workoutPlayToolbarTemplate,
    'text/html',
  ).body.childNodes;
  ...
  */
  ...
  let formatString;
  const toolbarIntervalReturn = setInterval(() => {
    //끝나는 조건
    if (
      workoutMinute == totalMinuteCount &&
      workoutLeftSecond == leftSecondCount
    ) {
...
export function setWorkoutPlayList() {
  ...
  const workoutPlayListDomTempate = new DOMParser().parseFromString(
    workoutPlayListDom,
    'text/html',
  ).body.childNodes;
