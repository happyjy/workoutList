...
export function getWorkoutTotalTime(workoutRoutineIndex) {
  ...
  const sumSecond = workoutlist
    .map((v, index, arr) => {
      const gapTimeBetweenWorkoutTimes =
        parseInt(v.times) != 1 ? parseInt(v.times) - 1 : 0;
      const gapTimeBetweenWorkout = index != arr.length ? 1 : 0;
      return (
        parseInt(v.second) * parseInt(v.times) +
        gapTimeBetweenWorkoutTimes +
        gapTimeBetweenWorkout
      );
    })
    .reduce((acc, cur) => {
      return acc + cur;
